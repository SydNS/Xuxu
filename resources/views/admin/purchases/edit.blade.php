    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.purchases.update", [$purchase->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('product_purchased') ? 'is-invalid' : '' }}" name="product_purchased_id" id="product_purchased_id" required>
                    @foreach($product_purchaseds as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_purchased_id') ? old('product_purchased_id') : $purchase->product_purchased->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number" name="quantity" id="quantity" value="{{ old('quantity', $purchase->quantity) }}" step="1">
